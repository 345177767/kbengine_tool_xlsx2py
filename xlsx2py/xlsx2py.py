...
g_dctDatas = {} # 表格的所有数据
g_dctDatasSN = {} # 表格的所有Server Need数据
g_dctDatasCN = {} # 表格的所有Client Need数据
g_fdatas = {}
...
#####################文件头检测#######################
	...
	def __checkDefine(self):
		"""
		第一行的各元素是否符合定义格式"name[signs][func]"以及key是否符合规定
		"""
	...
	def sheetIndex2Data(self):
		...
		for index in self.__exportSheetIndex:
			SheetName = self.xbook.getSheetNameByIndex(index)
			sheetName = SheetName[SheetName.find(EXPORT_PREFIX_CHAR)+1:].strip()
...
##############符号字典的相关设置EXPORT_SIGN###################
	...
	def isServerNeed(self, cellData):
		pass

	def isClientNeed(self, cellData):
		pass
...
###############export to  py部分######################
	...
	def writeBody(self):
		...
		for dataName, datas in g_dctDatas.items():
			...
			# 生成lua
			luahandle = codecs.open(self.fileHandler.stream.name + "." + dataName + ".lua", "w+",'utf-8')
			bytes = ''
			for it in jsontolua.dic_to_lua_str(datas):
				bytes += it
			luahandle.write("%s = %s" % (dataName, (bytes[1:])))
			luahandle.close()
		'''

		for dataName, datas in g_dctDatasSN.items():
			stream = dataName + "="
			#stream += xlsxtool.dict_to_text(datas) + "\n"
			stream += "%s\n" % (datas)
			self.xlsxWrite(stream)
		...
		# 生成lua
		luahandle = codecs.open(self.fileHandler.stream.name[0:-3] + ".lua", "w+",'utf-8')			
		for dataName, datas in g_dctDatasCN.items():
			bytes = ''
			for it in jsontolua.dic_to_lua_str(datas):
				bytes += it
			luahandle.write("%s = %s\n" % (dataName, (bytes[0:])))
...
EXPORT_SIGN['s'] = xlsx2py.isServerNeed
EXPORT_SIGN['c'] = xlsx2py.isClientNeed
